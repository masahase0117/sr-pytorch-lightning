...
class EDSR(nn.Module):
    ...
    def __init__(self, scale_factor=4, ngf=256, n_blocks=32, res_scale=0.1):
        ...
        self.head = nn.Sequential(
            nn.ReflectionPad2d(1), nn.Conv2d(3, ngf, kernel_size=3),
        )
        self.body = nn.Sequential(
        ...
        self.register_buffer(
            name="mean",
            tensor=torch.tensor(
                [[[0.4488]], [[0.4371]], [[0.4040]]], requires_grad=False
            ),
        )
...
class Discriminator(nn.Sequential):
    ...
    def __init__(self, ndf):
        ...
        super(Discriminator, self).__init__(
            ...
            *conv_block(ndf, ndf * 2, 1),
            *conv_block(ndf * 2, ndf * 2, 2),
            *conv_block(ndf * 2, ndf * 4, 1),
            *conv_block(ndf * 4, ndf * 4, 2),
            *conv_block(ndf * 4, ndf * 8, 1),
            *conv_block(ndf * 8, ndf * 8, 2),
